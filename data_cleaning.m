...
%% EMPTY VALUES
% col1 = find(isnan(alldata(:,1)) == 1);
% col2 = find(isnan(alldata(:,2)) == 1);
% col3 = find(isnan(alldata(:,3)) == 1);
%% FIND THE SINGLE STRIKE 
alldata = sortrows(alldata,[1,2,4]);
single_k = [];
for i = 1:size(alldata,1) - 1
    if alldata(i,3) == alldata(i + 1,3)
        single_k = cat(2, single_k, i);
    end
end
