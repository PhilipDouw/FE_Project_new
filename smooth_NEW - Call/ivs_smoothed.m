function [u, tau, g, gamma] = ivs_smoothed(call_price, close, strike, ...
...
% Input:
% * call_price   = (Nx1)  = market prices of call options
...
if~(size(call_price) == size(close) == size(strike) == size(maturity) == ...
...
if isempty(implied_volatility)
    implied_volatility = blsimpv(close, strike, interestrate, maturity, call_price, [], dividendyield);
end
...
[call_price, kappa, tau, forward_tau, interestrate_tau, dividendyield_tau] = ...
